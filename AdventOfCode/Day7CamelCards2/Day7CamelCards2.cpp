// Day7CamelCards2.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
static size_t determineType(const std::string& hand) {
	...
	if (auto it = mapHand.find('J'); it != mapHand.end()) {
		...
		for (const auto& [key, value] : mapHand) {
			if (value > maxVal) {
				maxVal = value;
				maxChar = key;
			}
		...
		mapHand[maxChar] += amount;
		typeHand = mapHand.size() * 2;
	}
...
static int translateStrength(char c) {
	if (c == 'A') return 1;
	if (c == 'K') return 2;
	if (c == 'Q') return 3;
	if (c == 'J') return 14; // Weakest card
	if (c == 'T') return 5;
	return 15 - (c - '0');
}

static bool determinesStrength(const std::pair<size_t, std::pair<std::string, int>>& lhs,
...
int main()
{
	...
	for (const auto& hand : hands) {
		results += static_cast<long long>(hand.second.second) * static_cast<long long>(id);
		--id;
	}
